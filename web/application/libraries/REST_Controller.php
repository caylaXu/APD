...
abstract class REST_Controller extends CI_Controller {
    ...
    /**
     * elicit HTTP header information
     * @var array
     */
    protected $header = [];
...
//        require_once ( APPPATH . "../phplibs/hpf_exception/libraries/set_my_exception_handler.php" );
        ...
        $this->_args = array_merge(
            (array)$this->_get_args,
            (array)$this->_options_args,
            (array)$this->_patch_args,
            (array)$this->_head_args,
            (array)$this->_put_args,
            (array)$this->_post_args,
            (array)$this->_delete_args,
            (array)$this->{'_' . $this->request->method . '_args'}
        );
...
//        }
    ...
    protected function _parse_post()
    {
        ...
        {
            $this->_post_args = json_decode(file_get_contents('php://input'),true);
        }
        else
        {
    ...
    {
        $this->_put_args = json_decode(file_get_contents('php://input'),true);
        if(!is_array($this->_put_args))
        {
            $this->_put_args = array();
        }
...
//        }
    ...
    {
        $this->_delete_args = json_decode(file_get_contents('php://input'),true);
        if ($this->input->method() === 'delete' && empty($this->_delete_args))
        {
    ...
    public function header($key = NULL)
    {
        ...
        return array_key_exists(strtoupper($key), $this->header) ? $this->header[strtoupper($key)] : FALSE;
    }
